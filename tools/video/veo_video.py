...
class VeoVideo(BaseTool):
    ...
    def _execute_google(self, inputs: dict[str, Any]) -> ToolResult:
        ...
        try:
            ...
            if not is_vertex:
                # Files API exists only on the Gemini Developer API backend.
                client.files.download(file=video_asset)
            elif not video_asset.video_bytes:
                return ToolResult(
                    success=False,
                    error="Vertex AI returned a video without inline bytes "
                    f"(uri={getattr(video_asset, 'uri', None)!r}). Configure the "
                    "request without an output GCS URI so bytes are returned inline.",
                )
